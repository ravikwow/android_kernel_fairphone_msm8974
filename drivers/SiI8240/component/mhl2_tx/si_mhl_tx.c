...
#include "queue.h"
#include <asm/string.h>  //wangzhanmeng modify for compile error 20130114
...
uint16_t hTotal, vTotal;
    ...
    PP_DEBUG_PRINT(("SiiMhlTxFindTimingsFromTotals: hTotal: %d vTotal:%d\n",hTotal,vTotal));
    return retVal;
}
...
uint8_t SiiMhlTxIncomingInfoFrameChanged(PAVIInfoFrame_t pAviInfoFrame)
{
    ...
    inputVideoCode          = pAviInfoFrame->payLoad.hwPayLoad.namedIfData.ifData_u.bitFields.VIC.VIC;
    SiiMhlTxDrvSetInputVideoCode(inputVideoCode);

	//VIDEO_CAPABILITY_D_BLOCK_found=pAviInfoFrame->payLoad.hwPayLoad.namedIfData.ifData_u.bitFields.pb3.RGBQuantizationRange;
	//TXC_DEBUG_PRINT(("@@@@@@@@@@@@@@@@@@@@@@@@VIDEO_CAPABILITY_D_BLOCK_found=0x%x\n",VIDEO_CAPABILITY_D_BLOCK_found));

    if(0 != infoFrameCmp(&mhlTxConfig.currentAviInfoFrame,pAviInfoFrame))//&&((uint16_t)pAviInfoFrame->payLoad.hwPayLoad.namedIfData.ifData_u.bitFields.VIC.VIC!=0))
    {
    uint8_t checkSum=0;
        ...
        if (0 != checkSum)
        {
            TXC_DEBUG_PRINT(("MhlTx: ERROR!!! AVI InfoFrame CheckSum: %d\n",(uint16_t)checkSum ));
        }
        else
        {
            TXC_DEBUG_PRINT(("MhlTx:SiiMhlTxIncomingInfoFrameChanged: VIC: %d, pAviInfoFrame->header.type_code:0x%02X\n",
					(uint16_t)pAviInfoFrame->payLoad.hwPayLoad.namedIfData.ifData_u.bitFields.VIC.VIC, (uint16_t)pAviInfoFrame->header.type_code));

            // Disable HDCP
            if (0 != mhlTxConfig.currentAviInfoFrame.header.type_code)
            {
             TXC_DEBUG_PRINT(("MhlTx:SiiMhlTxIncomingInfoFrameChanged:  mhlTxConfig.currentAviInfoFrame.header.type_code: %d\n",(uint16_t)0 != mhlTxConfig.currentAviInfoFrame.header.type_code));
	...
	#if 0
	else if((uint16_t)pAviInfoFrame->payLoad.hwPayLoad.namedIfData.ifData_u.bitFields.VIC.VIC==0){
		TXC_DEBUG_PRINT(("MhlTx:  AVI-IF can't be accessed sucessful^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n"));
    ...
    {
        TXC_DEBUG_PRINT(("MhlTx:  AVI-IF no change\n"));
    }
    return 0;
}
#else
uint8_t SiiMhlTxIncomingInfoFrameChanged(PAVIInfoFrame_t pAviInfoFrame)
{
	...
	inputVideoCode          = mhlTxConfig.currentAviInfoFrame.payLoad.hwPayLoad.namedIfData.ifData_u.bitFields.VIC.VIC;
	SiiMhlTxDrvSetInputVideoCode(inputVideoCode);
...
uint8_t packedPixelNeeded=0;
	...
	TXC_DEBUG_PRINT(("print mhlTxConfig.currentAviInfoFrame:::::::::::::::::\n"));
	DumpIncomingInfoFrame(&(mhlTxConfig.currentAviInfoFrame),sizeof(mhlTxConfig.currentAviInfoFrame));
    if ( 0x82 != mhlTxConfig.currentAviInfoFrame.header.type_code)
    {
        TXC_DEBUG_PRINT(("unexpected type code:%d\n",(uint16_t)aviInfoFrame.header.type_code));
    ...
    else
    {
        ...
        TXC_DEBUG_PRINT(("MhlTx:SiiMhlTxAttemptPackedPixel: VIC: %d\n",(uint16_t)mhlTxConfig.currentAviInfoFrame.payLoad.hwPayLoad.namedIfData.ifData_u.bitFields.VIC.VIC));
    ...
    if (0 == pixelClockFrequency)
    {
        ...
        //packedPixelNeeded=0;
	packedPixelNeeded=1;//Garyyuan add for display the picture when AVI can't be accessed
    }
    else
    {
    #ifdef THREE_D_SUPPORT //(
	...
	*/
	...
	TXC_DEBUG_PRINT(("packedPixelNeeded=%d&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&\n",packedPixelNeeded));
    if (PackedPixelAvailable)
    {
                                        ...
                                        )
{
    ...
    else
    {
        ...
        uint8_t *pucLongDescriptors;
            ...
            TX_EDID_PRINT(("misc support index-> %02x\n",*((uint8_t *)&pCeaExtensionVersion3->miscSupport) ));
                ...
                case dbtcVideoDataBlock:
                    {
                        ...
                        TX_EDID_PRINT(("EDID -> %d descriptors in Short Descriptor Video Block\n",i));
                    }
                    break;
                        ...
                        switch (ExtendedTagCode.extendedTagCode)
                        {
                            case etcVideoCapabilityDataBlock:
                                ...
                                PVideoCapabilityDataPayLoad_t pPayLoad = &pVideoCapabilityDataBlock->payload;
                					...
                					TX_EDID_PRINT(("EDID -> Short Descriptor Video Capability Block found\n"));
                                }
            					break;
...
static bool_t SiiMhlTxParseDetailedTiming(P_18ByteDescriptor_u pDesc , uint8_t Block, uint8_t *pIsTiming,PMhl2VideoDescriptor_t pMhl2VideoDescriptor)
{
	...
	uint8_t i;
	uint16_t TmpWord;
	TX_EDID_PRINT(("SiiMhlTxParseDetailedTiming...\n"));

#ifdef ENABLE_TX_EDID_PRINT //(
    {
    uint8_t *pucTemp=(uint8_t *) pDesc;
        TX_EDID_PRINT(("address: %x detailed timing:\n",pDesc));
        for (i=0; i < sizeof(*pDesc);++i)
        {
            SiiOsDebugPrintSimpleAlways(" %02x",pucTemp[i]);
        }
    ...
    *pIsTiming = 0;
    ...
    uint32_t verticalRefreshRateInMilliHz,horizontalSyncFrequencyInHz,horizontalSyncPeriodInPixels;
        ...
        *pIsTiming = 1;
		...
		PIXCLK_DEBUG_PRINT(("Horizontal Active Pixels: %d;pDesc->dtd.horzActive7_0=0x%x;pDesc->dtd.horzActiveBlankingHigh.horzActive11_8=0x%x\n", columns,pDesc->dtd.horzActive7_0,
			pDesc->dtd.horzActiveBlankingHigh.horzActive11_8));
...
PCeaExtension_t pCeaExtension = (PCeaExtension_t)pEdidBlockData;
    ...
    union
        ...
        {
        uint8_t isTiming=0;
			TXC_DEBUG_PRINT(("Drv: SiiMhlTxParse861LongDescriptors\n"));
...
PEDID_Block0_t pEdidBlock0 = (PEDID_Block0_t)&edidBlockData[0];
uint8_t dtdLimit;
uint8_t	VSDBlengthFollowingHeader=0;
...
union
    ...
    P_18ByteDescriptor_u pLongDescriptors;
		...
		DumpEdidBlock(edidBlockData,128)  // no semicolon needed here

    if (EDID_EXTENSION_BLOCK_MAP == pCeaExtension->tag)
    {
        pBlockMap = (PBlockMap_t)pCeaExtension;
        pCeaExtension++;//advance to next block
    }
    TXC_DEBUG_PRINT(("MhlTx: SiiMhlTxPruneEDID\n"));
	...
	VSDBlengthFollowingHeader=mhlTxConfig.pHdmiVendorSpecificDataBlock->header.lengthFollowingHeader;
	TX_PRUNE_PRINT(("mhlTxConfig.pHdmiVendorSpecificDataBlock->header.lengthFollowingHeader=0x%x,VSDBlengthFollowingHeader0x%x\n",
					mhlTxConfig.pHdmiVendorSpecificDataBlock->header.lengthFollowingHeader,VSDBlengthFollowingHeader));
	...
	TX_PRUNE_PRINT(("sizeof(pEdidBlock0->detailedTimingDescriptors):0x%02x\n, sizeof(pEdidBlock0->detailedTimingDescriptors[0]:0x%02x, dtdLimit:0x%02x\n",
		(uint16_t)sizeof(pEdidBlock0->detailedTimingDescriptors),sizeof(pEdidBlock0->detailedTimingDescriptors[0]),dtdLimit));
    ...
    if (mhlTxConfig.pHdmiVendorSpecificDataBlock){
		if (mhlTxConfig.pByte13ThroughByte15)
	    {
			...
			/********************************Garyyuan add for sd not support for 1.2 dongle or sink*********/
						  
			if (0x20 > mhlTxConfig.devCapCache.aucDevCapCache[DEVCAP_OFFSET_MHL_VERSION]){
			  mhlTxConfig.pByte13ThroughByte15->byte13._3D_present=0;
			  mhlTxConfig.pByte13ThroughByte15->byte13._3D_Multi_present=0;
			  mhlTxConfig.pByte13ThroughByte15->vicList[1]=0;
			}
			/********************************Garyyuan add for sd not support for 1.2 dongle or sink*********/
	        ...
	        uint8_t outerLoopLimit;
				...
				//TX_PRUNE_PRINT(("trace44444444444*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&\n"));
    ...
    uint8_t innerLoopLimit = mhlTxConfig.pVideoDataBlock2D->header.lengthFollowingHeader;
        ...
        uint8_t outerLoopLimit = innerLoopLimit;
            ...
            for (i=0; i < outerLoopLimit;)
            {
            	TX_PRUNE_PRINT(("PackedPixelAvailable:%d, mhlTxConfig.pVideoDataBlock2D->shortDescriptors[%d].VIC=%d\n"
						, (uint16_t)PackedPixelAvailable
						,i
						,mhlTxConfig.pVideoDataBlock2D->shortDescriptors[i].VIC));
		...
		else if (!PackedPixelAvailable&&(0x10 == (mhlTxConfig.pVideoDataBlock2D->shortDescriptors[i].VIC&0x7F)))
                {
                	uint8_t j,prev;
                    numCeaVicsPruned++;
                    for (prev=i,j = i+1; j < innerLoopLimit;++j,++prev)
		...
		else if (!PackedPixelAvailable&&(0x1F == (mhlTxConfig.pVideoDataBlock2D->shortDescriptors[i].VIC&0x7F)))
                {
                	uint8_t j,prev;
                    numCeaVicsPruned++;
                    for (prev=i,j = i+1; j < innerLoopLimit;++j,++prev)
	...
	//TX_PRUNE_PRINT(("trace77777777777777*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&\n"));
	...
	//DumpEdidBlock(edidBlockData,sizeof(*edidBlockData))  // no semicolon needed here
...
{
int flag=0;
	TXC_DEBUG_PRINT(("MhlTx: Check3D_DTD_Sequence_Done....\n"));
	...
	//if (flag)
	{
       ...
       // if (TestMiscFlag(FLAGS_BURST_3D_VIC_DONE))
        {
        	TXC_DEBUG_PRINT(("MhlTx: FLAGS_BURST_3D_VIC_DONE....\n"));
           // if (!TestMiscFlag(FLAGS_BURST_3D_DONE))
            {
            	TXC_DEBUG_PRINT(("MhlTx: FLAGS_BURST_3D_DONE....\n"));
                SiiMhlTxDisplayTimingEnumerationEnd();
            }
        }
    }
...
uint8_t dtdLimit = (uint8_t)pCeaExtension->version_u.version3.miscSupport.totalNumberDetailedTimingDescriptorsInEntireEDID;
	TX_EDID_PRINT(("SiiMhlTxProcess3D_DTDBurst...\n"));
    ...
    PEDID_Block0_t pEdidBlock0 = (PEDID_Block0_t)&edidBlockData[0];
        ...
        bool_t isValid=0;
		TX_EDID_PRINT(("SiiMhlTxProcess3D_DTDBurst22222222222...\n"));
            ...
            if (isTiming)
            {
				...
				#if 0  // comment by Garyyuan because these words will change the DTD timing and make the TV can't display well.
                else
                {
    ...
    union
        ...
        bool_t isValid=0;
		TX_EDID_PRINT(("SiiMhlTxProcess3D_DTDBurst333333333...\n"));
...
uint8_t isTiming=0;
    ...
    {	TXC_DEBUG_PRINT(("Drv: SiiMhlTxParseBlockZeroTimingDescriptors\n"));
...
void SiiMhlTxMakeItDVI(PEDID_Block0_t pEdidBlock0)
{
    ...
    EDID_Data.HDMI_Sink = false;
        ...
        uint16_t counter;
    		...
    		ERROR_DEBUG_PRINT(("EDID ERROR...Setting second block to 0xFF %d\n",(uint16_t)EDID_REV_ADDR_ERROR));
...
SiiMhlTxDrvGetEdidBlockResult_e ddcResult;
    ...
    PEDID_Block0_t pEdidBlock0 = (PEDID_Block0_t)pEdidBlockData;
        ...
        {
            TXC_DEBUG_PRINT(("sizeof(CeaExtension_t):0x%x\n",(unsigned int)sizeof(CeaExtension_t)));
            TXC_DEBUG_PRINT(("sizeof(CeaExtensionVersion1_t):0x%x\n",(unsigned int)sizeof(CeaExtensionVersion1_t)));
            TXC_DEBUG_PRINT(("sizeof(CeaExtensionVersion2_t):0x%x\n",(unsigned int)sizeof(CeaExtensionVersion2_t)));
            TXC_DEBUG_PRINT(("sizeof(CeaExtensionVersion3_t):0x%x\n",(unsigned int)sizeof(CeaExtensionVersion3_t)));
            TXC_DEBUG_PRINT(("sizeof(DataBlockHeaderByte_t):0x%x\n" ,(unsigned int)sizeof(DataBlockHeaderByte_t )));
...
uint8_t SiiMhlTxGetNumCea861Extensions(uint8_t blockNumber)
{
PEDID_Block0_t pEdidBlock0 = (PEDID_Block0_t)&edidBlockData;
uint8_t limitBlocks = sizeof(edidBlockData)/EDID_BLOCK_SIZE;
	TXC_DEBUG_PRINT(("Mhl2Tx: blockNumber: %d#############\n",(uint16_t)blockNumber));

    SiiMhlTxDrvGetEdidFifoNextBlock(&edidBlockData[EDID_BLOCK_SIZE * blockNumber]);
...
static void SiiMhlTxInitiateEdidSequence(void)
{
    ...
    uint8_t result;
        ...
        else
        {
            ...
            SetMiscFlag(SiiMhlTxInitiateEdidSequence,FLAGS_EDID_READ_DONE);
            SiiMhlTxTmdsEnable();
        }
#else //)(
        /*
        ...
        */
        ...
        {
            mhlTxConfig.flags.edidLoopActive=1;
...
void	SiiMhlTxMscCommandDone( uint8_t data1 )
{
    ...
    {
    #ifndef REQUEST_EDID_ATOMICALLY //(
    uint8_t lastEdidBlock = mhlTxConfig.mscLastOffset;
	#endif
        mhlTxConfig.mscLastCommand = 0;
        mhlTxConfig.mscLastOffset  = 0;
        MhlTxDriveStates();
        ...
        EDID_DEBUG_PRINT(("Entire EDID Read complete\n"));
            ...
            if (Result != EDID_OK)
            {
            	...
            	else
            	{
                    ...
                    /*
                        2012-08-13
                        The data is not modified in this case anyway.
                            'no reason to re-load the buffer
                    */
            	}
                // number of extensions is one less than number of blocks
                mhlTxConfig.numEdidExtensions = 0;
                SiiMhlTxMakeItDVI(pEdidBlock0);
        		Result = EDID_OK;
            }
            else
            ...
            uint8_t counter;
                ...
                mhlTxConfig.flags.edidLoopActive=0;
                SiiMhlTx3dReqForNonTranscodeMode();
            }


    #else //)(
        uint8_t readNextBlock=0;
            EDID_DEBUG_PRINT(("EDID Read complete -- getting block: %d\n",(uint16_t)lastEdidBlock));
            SiiMhlTxDrvGetEdidFifoNextBlock(&edidBlockData[EDID_BLOCK_SIZE * lastEdidBlock]);
            if (0 == lastEdidBlock)
            {
                Result = SiiMhlTxParseEDID (pEdidBlock0);
            ...
            {
                Result = SiiMhlTxParse861Block(&edidBlockData[EDID_BLOCK_SIZE * lastEdidBlock]);
                if (EDID_OK != Result)
                {
                	ERROR_DEBUG_PRINT(("EDID -> Extension Parse FAILED: Result:%d\n",(uint16_t)Result));
                    SiiMhlTxMakeItDVI(pEdidBlock0);
            ...
            if (lastEdidBlock< mhlTxConfig.numEdidExtensions)
            {
                EDID_DEBUG_PRINT(("last block: %d  max:%d\n"
                    ,(uint16_t)lastEdidBlock
                    ,(uint16_t)mhlTxConfig.numEdidExtensions
                    ));
                if (readNextBlock)
                {
                    SiiMhlTxRequestEdidBlock( lastEdidBlock+ 1);
                }
            }
            else
            {
                /* done with EDID reading/parsing */
                mhlTxConfig.flags.edidLoopActive=0;
                SiiMhlTx3dReqForNonTranscodeMode();
            }
    #endif //)
...
void	SiiMhlTxNotifyConnection( bool_t mhlConnected )
{
	...
	else
	{
        ...
        mhlTxConfig.mhlHpdRSENflags &= ~MHL_RSEN;
		memset(&mhlTxConfig.currentAviInfoFrame,0,sizeof(AVIInfoFrame_t));
	}
	MhlTxProcessEvents();
}
...
static void	MhlTxResetStates( void )
{
	memset(&mhlTxConfig,0,sizeof(mhlTxConfig));
	...
	CBusQueue.header.head = 0;
	CBusQueue.header.tail = 0;
	TXC_DEBUG_PRINT (("MhlTx:MhlTxResetStates....\n" ));
}
...
void DumpIncomingInfoFrameImpl(PInfoFrame_t pInfoFrame,uint8_t length)
{
uint8_t j;
uint8_t *pData = (uint8_t *)pInfoFrame;
    TXD_DEBUG_PRINT(("MhlTx:pInfoFrame length:0x%02x -----\n",length));
...
void SiiMhlTxProcessAviInfoFrameChange(PAVIInfoFrame_t pAviInfoFrame)
{
    ...
    {
        TXC_DEBUG_PRINT(("MhlTx: SiiMhlTxProcessAviInfoFrameChange: VIC: %d\n",(uint16_t)mhlTxConfig.currentAviInfoFrame.payLoad.hwPayLoad.namedIfData.ifData_u.bitFields.VIC.VIC));
