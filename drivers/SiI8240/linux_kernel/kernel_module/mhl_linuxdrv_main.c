...
/** ZTE_MODIFY niugang10089953 modify for P897A23 MHL **/
int sil8240_reset_gpio = 0;
int sil8240_int_gpio = 0;
int ap_usb_sel0 = 0;
int ap_usb_sel1 = 0;
/** ZTE_MODIFY niugang10089953 end **/
//static struct device *siiMhlClassDevice;
/** ZTE_MODIFY niugang10089953 modify for P897A23 MHL **/
int SiI8240_power_up(int on,struct device *dev)
{
	int rc;
	static struct regulator*reg_SiI8240_1p2v;	
	static struct regulator*reg_SiI8240_3v;
	//static int has_power_on = 0;

	
	if(reg_SiI8240_1p2v == NULL){
        	reg_SiI8240_1p2v = regulator_get(dev,"8941_l2");
        	if (IS_ERR_OR_NULL(reg_SiI8240_1p2v)) 
        	{
        		pr_err("could not get reg_SiI8240_1p2v, rc = %ld\n",
        		       PTR_ERR(reg_SiI8240_1p2v));
        		return -ENODEV;
        	}
	}

	if(reg_SiI8240_3v == NULL){
        	reg_SiI8240_3v = regulator_get(dev,"8941_l22");
        	if (IS_ERR_OR_NULL(reg_SiI8240_3v)) 
        	{
        		pr_err("could not get reg_SiI8240_1p2v, rc = %ld\n",
        		       PTR_ERR(reg_SiI8240_3v));
        		return -ENODEV;
        	}
	}
    	...
    	if (on)
	{
		...
		rc = regulator_enable(reg_SiI8240_3v); 
		if (rc) 
		{
			pr_err("enable reg_SiI8240_3v failed, rc=%d\n", rc);
			return -ENODEV;
		}
		msleep(50);
		rc = regulator_enable(reg_SiI8240_1p2v); 
		if (rc) 
		{
			pr_err("enable reg_SiI8240_1p2v failed, rc=%d\n", rc);
			return -ENODEV;
		}
		//msleep(50);

		//gpio_set_value_cansleep(SiI8240_AVCC33_EN, 1);
      }
	else
	{

		rc = regulator_disable(reg_SiI8240_1p2v);
		if (rc)
		{
			pr_err("disable reg_SiI8240_1p2v failed, rc=%d\n", rc);
			return -ENODEV;
		}
		msleep(50);
		rc = regulator_disable(reg_SiI8240_3v);
		if (rc)
		{
			pr_err("disable reg_SiI8240_3v failed, rc=%d\n", rc);
			return -ENODEV;
		}
		//gpio_set_value_cansleep(SiI8240_AVCC33_EN, 0);
      }
	return 0;

}
void GpioSetTxResetPin(int val)
{
	gpio_direction_output(sil8240_reset_gpio,val);
}

void  SiI8240_setup_irq_gpio(struct device *dev)
{
	int status = 0;
	...
	sil8240_int_gpio = of_get_named_gpio(of_node, "mhl-intr-gpio", 0);
	if (sil8240_int_gpio < 0) {
		pr_err("%s: Can't get mhl-rst-gpio\n", __func__);
		return;
	}

	status = gpio_request(sil8240_int_gpio, "mhl-intr-gpio");
	if (status) {
		pr_err("%s:Failed to request GPIO %d\n",
					__func__, sil8240_int_gpio);
		return;			
	}
	status = gpio_direction_input(sil8240_int_gpio);
	if (status) {
	...
	sil8240_reset_gpio = of_get_named_gpio(of_node, "mhl-rst-gpio", 0);
	if (sil8240_reset_gpio < 0) {
		pr_err("%s: Can't get mhl-rst-gpio\n", __func__);
		return;
	}

    status = gpio_request(sil8240_reset_gpio, "SiI8240_reset");
	if (status) {
		pr_err("%s:Failed to request GPIO %d\n",
					__func__, sil8240_reset_gpio);
		return;			
	}
	status = gpio_direction_output(sil8240_reset_gpio,1);
	msleep(5);
	if (status) {
		pr_err("%s:Failed to configure GPIO %d\n",
				__func__, sil8240_reset_gpio);
		goto gpio_free_reset;		
	}

	ap_usb_sel0 = of_get_named_gpio(of_node, "mhl-usb_sel0", 0);
	if(ap_usb_sel0 < 0){
		pr_err("%s: Can't get mhl-usb_sel0\n", __func__);
		return;
	}
#if 1	
	status = gpio_request(ap_usb_sel0, "AP_USB_SEL0");
	if (status) {
		pr_err("%s:Failed to request GPIO %d\n",
					__func__, ap_usb_sel0);
		return;			
	}
#endif
	ap_usb_sel1 = of_get_named_gpio(of_node, "mhl-usb_sel1", 0);
	if(ap_usb_sel1 < 0){
		pr_err("%s: Can't get mhl-usb_sel1\n", __func__);
		return;
	}
#if 1	
	status = gpio_request(ap_usb_sel1, "AP_USB_SEL1");
	if (status) {
		pr_err("%s:Failed to request GPIO %d\n",
					__func__, ap_usb_sel1);
		return;			
	}
#endif
gpio_free_int:
	gpio_free(sil8240_int_gpio);
gpio_free_reset:
	gpio_free(sil8240_int_gpio);
}
/** ZTE_MODIFY niugang10089953 end **/
 ...
 *
 *****************************************************************************/
 //#define SiI8240_RESET_GPIO	8/** ZTE_MODIFY chenfei added for mhl SiI8240 driver 2012/12/17 */
extern void GpioSetTxResetPin(int val);
void AppResetMhlTx(uint16_t hwResetPeriod,uint16_t hwResetDelay)
{

	// Reset the TX chip,
	
	//Here you shuold set your reset signal to high;
	//HalGpioSetTxResetPin(LOW);  
	GpioSetTxResetPin(0);/** ZTE_MODIFY chenfei added for mhl SiI8240 driver 2012/12/17 */
	
	HalTimerWait(hwResetPeriod);

	//Here you shuold set your reset signal to low;
	//HalGpioSetTxResetPin(HIGH);
	GpioSetTxResetPin(1);/** ZTE_MODIFY chenfei added for mhl SiI8240 driver 2012/12/17 */
