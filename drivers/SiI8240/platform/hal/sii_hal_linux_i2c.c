...
/** ZTE_MODIFY chenfei added for mhl SiI8240 driver 2012/12/17 */
 extern  int    SiI8240_power_up(int on,struct device *dev);
 /** ZTE_MODIFY niugang10089953 modify for P897A23 MHL **/
 extern   void  SiI8240_setup_irq_gpio(struct device *dev);
...
static int32_t MhlI2cProbe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("chenfei %s\n", __func__);//wangzhanmeng for test 20130121

	pr_info("chenfei %s\n", __func__);
	/** ZTE_MODIFY niugang10089953 modify for P897A23 MHL **/
	 if  (client != NULL) 
	 {
	 	SiI8240_power_up(1,&client->dev);
	   	SiI8240_setup_irq_gpio(&client->dev);
	 }
	/** ZTE_MODIFY niugang10089953 end **/
    return 0;
}
...
halReturn_t HalOpenI2cDevice(char const *DeviceName, char const *DriverName)
{
	...
	/** ZTE_MODIFY niugang10089953 modify for P897A23 MHL **/
    #ifdef CONFIG_OF
    gMhlDevice.driver.driver.of_match_table = mhl_match_table;
    #endif
	/** ZTE_MODIFY niugang10089953 end **/
